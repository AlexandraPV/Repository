...
void server_answer(http_request_t req, socket_t * clientSocket,lanser ** freelanser)
{
    puts(req.method);
    puts(req.uri);
    if (!strcmp(req.uri, "/Freelancer-count"))
    {
...
void server_start_page(socket_t* clientSocket)
{
    char text[]="<html>"
                ...
                "<body>"
                "<h1>HELLO Word</h1>"
                "<p><a href=""/Freelancer/"">Freelancers</a></p>"
                "</body>"
                "</html>";
...
void server_GET_html(socket_t* clientSocket,lanser ** freelancer)
{
    ...
    for(int i=0; i<10; i++)
    {
        if(Freelanser_name_get(freelancer[i])!=NULL)
        {
            sprintf(text,"<p><a href=""/Freelancer/%i"">%s %s</a></p>",i,Freelanser_name_get(freelancer[i]),Freelanser_surname_get(freelancer[i]));
            strcat(buf,text);
        }
    }
    strcat(buf,"<p><a href=""/Freelancer/paste"">New Freelancer</a></p>");
    strcat(buf,"</body>"
           "</html>");
    socket_write_string(clientSocket,buf);
}
